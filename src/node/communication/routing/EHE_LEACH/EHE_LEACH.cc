...
Define_Module(EHE_LEACH);

void EHE_LEACH::startup()
{	
...
void EHE_LEACH::fromApplicationLayer(cPacket *pkt, const char *destination)
{	
		if(!isSink)
	{
		string dst(destination);
		EHE_LEACH_Packet *netPacket = new EHE_LEACH_Packet("data packet", NETWORK_LAYER_PACKET);
		netPacket->setEHE_LEACH_PacketKind(EHE_LEACH_DATA_PACKET);
...
void EHE_LEACH::fromMacLayer(cPacket *pkt, int macAddress, double rssi, double lqi)
{

	EHE_LEACH_Packet *netPacket = dynamic_cast <EHE_LEACH_Packet*>(pkt);

	if (!netPacket)
		return;

	switch (netPacket->getEHE_LEACH_PacketKind()) {

		case EHE_LEACH_DATA_PACKET:{
...
void EHE_LEACH::timerFiredCallback(int index)
{
	...
	switch (index) {
		...
		case START_ROUND:
		{
			...
			{	
				...
				trace() << "EHE_LEACH Candidate: "<< self <<"\n";
				isCH=true;
				double c = 1;
		...
		case SEND_ADV:
		{	
		EHE_LEACH_Packet *crtlPkt = new EHE_LEACH_Packet("CH Adv. Packet", NETWORK_LAYER_PACKET);
			crtlPkt->setByteLength(advPacketSize);
			crtlPkt->setEHE_LEACH_PacketKind(EHE_LEACH_ADV_PACKET);
		...
		case JOIN_CH:
		{
			...
			{	
				CHcandidates.sort(cmpRssi2);
			EHE_LEACH_Packet *crtlPkt = new EHE_LEACH_Packet("CM join", NETWORK_LAYER_PACKET);
				crtlPkt->setEHE_LEACH_PacketKind(EHE_LEACH_JOIN_PACKET);
		...
		case MAKE_TDMA:
		{
			...
			{
			EHE_LEACH_Packet *crtlPkt = new EHE_LEACH_Packet("TDMA", NETWORK_LAYER_PACKET);
				crtlPkt->setByteLength(tdmaPacketSize);
				crtlPkt->setEHE_LEACH_PacketKind(EHE_LEACH_TDMA_PACKET);
				crtlPkt->setSource(SELF_NETWORK_ADDRESS);
				crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
...
void EHE_LEACH :: finishSpecific()
{
	
}

void EHE_LEACH ::sendAggregate()
{
	...
	{	
		...
		EHE_LEACH_Packet *aggrPacket = new EHE_LEACH_Packet("CH Agg. Packet", NETWORK_LAYER_PACKET);
		aggrPacket->setByteLength(dataPacketSize+4);
		aggrPacket->setEHE_LEACH_PacketKind(EHE_LEACH_DATA_PACKET);
...
double EHE_LEACH::getDistanceToBS()
{
	double dist=20;
	cModule *n; 
	VirtualMobilityManager *l1;
	VirtualMobilityManager *l2;
 	n = theSNModule->getSubmodule("node",0);
	l1 = check_and_cast<VirtualMobilityManager*>(n->getSubmodule("MobilityManager"));
	l2 = check_and_cast<VirtualMobilityManager*>(getParentModule()->getParentModule()->getSubmodule("MobilityManager"));


	double BSx = l1->getLocation().x;
	double BSy = l1->getLocation().y;

	double Myx = l2->getLocation().x;
	double Myy = l2->getLocation().y; 

	dist = sqrt (pow ((BSx - Myx),2) + pow ((BSy - Myy),2) );
	trace() << "Dist = " << dist <<"\n";

	return dist;
}
void EHE_LEACH::processBufferedPacket()
{
	string dst;
	CHInfo info = *CHcandidates.begin();
	if (isCH)
...
bool cmpRssi2(CHInfo a, CHInfo b){
